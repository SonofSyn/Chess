...
test("New Position not on board", t => {
    ...
    let err = t.throws(async () => {
        game = await executeMove(game, { x: 2, y: 2 }, { x: 9, y: 11 });
    }, undefined)
...
test("No gamepiece on selected position", t => {
    let game: Game = { gameId: "1", turn: 0, winner: "", gameBoard: initBoard() ,history:{movementLog:[],beatenLog:{white:[],black:[]}}}
    let err = t.throws(async () => {
        game = await executeMove(game, { x: 3, y: 3 }, { x: 3, y: 4 });
    }, undefined)
...
test("Wrong turn Black", t => {
    let game: Game = { gameId: "1", turn: 0, winner: "", gameBoard: initBoard() ,history:{movementLog:[],beatenLog:{white:[],black:[]}}}
    let err = t.throws(async () => {
        game = await executeMove(game, { x: 2, y: 7 }, { x: 2, y: 6 });
    }, undefined)
    t.is(err.message, "Spieler Weiß ist am Zug")
})

test("Wrong turn White", async t => {
    
    let game: Game = { gameId: "1", turn: 0, winner: "", gameBoard: initBoard() ,history:{movementLog:[],beatenLog:{white:[],black:[]}}}
    game = await executeMove(game, { x: 2, y: 2 }, { x: 2, y: 3 });
    let err = t.throws(async () => {
        game = await executeMove(game, { x: 3, y: 2 }, { x: 3, y: 3 });
    }, undefined)
